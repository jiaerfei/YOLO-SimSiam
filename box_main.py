...
# data
parser.add_argument("--data", default=r" ",
...
if __name__ == "__main__":
    ...
    # normal train
    # model = YOLO(model="yolo11n-seg.yaml", task="segment", verbose=True)
    ...
    # few train
    # model = YOLO(model="yolo11n-seg.yaml", task="segment", verbose=True).load(ps.pretrained)
    # train_result = model.train(data=ps.data, epochs=ps.epochs, imgsz=ps.input_size, batch=ps.batch_size,
    #                            name=ps.name, project=ps.project, patience=ps.patience,
    #                            optimizer=ps.optim, lr0=ps.lr0, cos_lr=ps.cos_lr, lrf=ps.lrf,
    #                            freeze=ps.frozen)
